...
class TestFun(TestSetup):
    ...
    def testPost(self):
        ...
        with app.app_context(), app.test_request_context():
            ...
            self.output(expect)
            self.assertTrue(
                self.dataContainFun(expect, loads(first.data)),
                Routes['vfun'] + " View: did not have the expected fun"
            )
            ...
            self.output(expect)
            self.assertTrue(
                len(loads(second.data)) > 0,
                "Some fun should have been returned"
            )
            self.assertTrue(
                self.dataContainFun(expect, loads(second.data)),
                Routes['vfun'] + " View: did not have the expected fun"
            )
            self.assertTrue(
                len(loads(first.data)) <= len(loads(second.data)),
                Routes['vfun'] + " View: filters fun should have less fun"
            )
