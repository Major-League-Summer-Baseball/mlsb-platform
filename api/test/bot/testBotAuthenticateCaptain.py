...
class testAuthenticateCaptain(TestSetup):
    ...
    def testMain(self):
        ...
        with app.app_context(), app.test_request_context():
            ...
            expect = team['team_id']
            rv = self.app.post(
                Routes['botcaptain'], json=data, headers=headers
            )
            ...
            self.assertEqual(
                rv.status_code, TeamDoesNotExist.status_code, error
            )
            self.assertEqual(expect, loads(rv.data), error)
            ...
            self.output(loads(rv.data))
            self.output(expect)
            url = Routes['botcaptain']
            error = f"{url} POST: name of captain does not match"
            self.assertEqual(
                rv.status_code, NotTeamCaptain.status_code, error
            )
            self.assertEqual(expect, loads(rv.data), error)
