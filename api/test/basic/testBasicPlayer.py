...
class TestPlayer(TestSetup):
    ...
    def testPlayerInvalidPost(self):
        ...
        with app.app_context(), app.test_request_context():
            ...
            error_message = (
                Routes['player'] + " POST: request with missing parameter"
            )
            ...
            result = {
                'details': 'Player - gender',
                'message': InvalidField.message
            }
            error_message = (
                Routes['player'] + " POST: request with invalid gender"
            )
            self.postInvalidTest(
                Routes['player'],
                params,
            ...
            result = {
                'details': 'Player - name', 'message': InvalidField.message
            }
            error_message = (
                Routes['player'] + " POST: request with invalid player name"
            )
            self.postInvalidTest(
                Routes['player'],
                params,
    ...
    def testPlayerListApi(self):
        ...
        with app.app_context(), app.test_request_context():
            ...
            error_message = (
                Routes['player'] + " GET Failed to return list of players"
            )
    ...
    def testPlayerApiGet(self):
        ...
        with app.app_context(), app.test_request_context():
            ...
            # valid Player id
            self.getTest(
                Routes['player'] + "/" + str(player['player_id']),
                SUCCESSFUL_GET_CODE,
    ...
    def testPlayerApiPutDuplicateEmail(self):
        ...
        with app.app_context(), app.test_request_context():
            ...
            result = {
                'details': player_two_email.strip().lower(),
                'message': NonUniqueEmail.message
            }
            error_message = Routes['player'] + ' PUT: Valid player update'
            self.putTest(
                Routes['player'] + "/" + str(player_one['player_id']),
                params,
